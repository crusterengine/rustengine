...
#Compile the program
cd ../../rustengine
cargo build --release --bin 03_hashmap_linked
...
#for i in {0..9}
#do
gtime -f "$(date +%Y-%m-%d\ %H:%M:%S),%e,%U,%S,%P,%M,%F,%R,%c,%w,"$number_of_iterations","$compile_filename","$input_filename"" "$compiled_program" "$input_file" "$number_of_iterations" "fellow" 2>> "$log_folder/results_rust.csv"
#done
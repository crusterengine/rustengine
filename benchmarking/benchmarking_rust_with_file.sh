...
mkdir -p "log_folder_rust_file_reader_new"

#This captures the path to where the log files should be located
log_folder="../../../benchmarking/log_folder_rust_file_reader_new"
...
gtime -f "$(date +%Y-%m-%d\ %H:%M:%S),%e,%U,%S,%P,%M,%F,%R,%c,%w,"$number_of_iterations"" ./rustengine_1point2 "$input_file" "$number_of_iterations" 2>> "$log_folder/results_rust.csv"
